...
edit_text = QTextEdit()
edit_text.setFont(QFont('Arial', 30)) 
...
label_note.setFont(QFont('Arial', 20)) 
...
layout_note_button.addWidget(button_create_note)
button_delete_note = QPushButton("Delete note")
layout_note_button.addWidget(button_delete_note)
button_save_note = QPushButton("Save note")
layout_top_right.addWidget(button_save_note)
...
button_create_tag = QPushButton("Create tag")
layout_tag_button.addWidget(button_create_tag)
button_delete_tag = QPushButton("Delete tag")
layout_tag_button.addWidget(button_delete_tag)
button_search_tag = QPushButton("Search note")
layout_bottom_right.addWidget(button_search_tag)
...
def create_tag():
    ...
    list_tag.clear()
    list_tag.addItems(tags)
button_create_tag.clicked.connect(create_tag)
...
def delete_tag():
    if len(list_note.selectedItems()) > 0 and len(list_tag.selectedItems()) > 0:
        ...
        list_tag.clear()
        list_tag.addItems(tags)
button_delete_tag.clicked.connect(delete_tag)

def search_tag():
    text_result = "Found in :\n"
    count = 0
    search_tag = edit_line.text()
    for note_name in note_data.keys():
        tags = note_data[note_name]["tags"]
        if (search_tag in tags):
            text_result += "  - " + note_name + "\n"
            count += 1
    
    if count == 0:
        text_result = "Found nothing"
    edit_text.setText(text_result)
...
def delete_note():
    if len(list_note.selectedItems()) > 0 :
        ...
        edit_text.clear()
        list_note.addItems(note_data.keys())
button_delete_note.clicked.connect(delete_note)

def create_note():
    inputDialog = QInputDialog()
    inputDialog.resize(400, 1)
    ...
    # ok, new_note_name = QInputDialog.getText(main_win, "Add note", "Note name:")
    ...
    list_note.clear()
    list_note.addItems(note_data.keys())
button_create_note.clicked.connect(create_note)
